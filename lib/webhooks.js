...
async function deleteCollection(collectionPath) {
  const batch = writeBatch(db); // Use a batch operation for efficiency
  const collectionRef = collection(db, collectionPath);
  const snapshot = await getDocs(collectionRef);

  // Iterate over all documents in the collection
  snapshot.forEach((doc) => {
    batch.delete(doc.ref); // Queue deletion of each document
  });
...
export async function deleteWebhook(id) {
  ...
  // Delete all subcollections of the account document
  const subcollections = await getDocs(collection(db, collName, accountId));
  await Promise.all(
    subcollections.docs.map((subcollection) =>
