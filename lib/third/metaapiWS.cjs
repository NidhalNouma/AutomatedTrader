...
async function modifyTradeByWHID(
  ...
  try {
    ...
    if (listOfTrades.length > 0)
      for (let i = 0; i < listOfTrades.length; i++) {
        const trade = listOfTrades[i];
        if (all || trade.clientId?.search(webhookId) > 0)
          if (trade.symbol === symbol) {
            if (trade.type == positionType) {
              ...
              try {
                console.log(trade, connection.modifyPosition.toString());
